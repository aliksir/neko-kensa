// 循環依存検出（Tarjan SCC）+ ルール違反チェック（dependency-cruiser 相当）
...
// Tarjan のアルゴリズムで強連結成分（SCC）を検出する。循環依存 = 要素数 > 1 の SCC
...
// ルールファイル (.neko-kensa.json) を読み込んで rules 配列を返す
...
// エッジ一覧をコンパイル済みルールの正規表現で照合して違反を収集する
