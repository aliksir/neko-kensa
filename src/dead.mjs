// エントリポイントから到達できないデッドファイルを BFS で検出する（Knip 相当）
...
// CLI --entry 指定 → package.json 自動検出 → フォールバック（全ファイル）の優先順でエントリを解決
...
// package.json の main/bin/exports フィールドからエントリポイントを自動検出
